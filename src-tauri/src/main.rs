...
/// Debounce guard for repeated shortcut-triggered secondary launches.
///
/// We keep two protections:
/// - Bounce guard: ignore any back-to-back event within a tiny window.
/// - Same-action debounce: ignore repeated show/show or hide/hide within a larger window.
///   (but still allow quick hide -> show for responsive UX)
static LAST_SHORTCUT_EVENT_MS: AtomicU64 = AtomicU64::new(0);
static LAST_SHORTCUT_EVENT_KIND: AtomicU8 = AtomicU8::new(0);

const SHORTCUT_BOUNCE_GUARD_MS: u64 = 120;
const SHORTCUT_SAME_ACTION_DEBOUNCE_MS: u64 = 650;

const EVENT_KIND_SHOW: u8 = 1;
const EVENT_KIND_HIDE: u8 = 2;
const EVENT_KIND_SETTINGS: u8 = 3;
const EVENT_KIND_EMOJI: u8 = 4;

fn detect_shortcut_event_kind(app: &AppHandle, argv: &[String]) -> u8 {
    if argv.iter().any(|arg| arg == "--settings") {
        return EVENT_KIND_SETTINGS;
    }

    if argv.iter().any(|arg| arg == "--emoji") {
        return EVENT_KIND_EMOJI;
    }
...
fn should_handle_shortcut_event(event_kind: u8) -> bool {
    let now_ms = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|d| d.as_millis() as u64)
        .unwrap_or(0);

    let last_ms = LAST_SHORTCUT_EVENT_MS.load(Ordering::Relaxed);
    let elapsed_ms = now_ms.saturating_sub(last_ms);

    // Guard against duplicated launch notifications from one physical keypress.
    if elapsed_ms < SHORTCUT_BOUNCE_GUARD_MS {
        return false;
    }

    let last_kind = LAST_SHORTCUT_EVENT_KIND.load(Ordering::Relaxed);
    let should_skip_same_action_debounce = event_kind == EVENT_KIND_SHOW;

    if !should_skip_same_action_debounce
        && last_kind == event_kind
        && elapsed_ms < SHORTCUT_SAME_ACTION_DEBOUNCE_MS
    {
        return false;
    }

    LAST_SHORTCUT_EVENT_MS.store(now_ms, Ordering::Relaxed);
    LAST_SHORTCUT_EVENT_KIND.store(event_kind, Ordering::Relaxed);
    true
}

fn is_unity_or_ubuntu_desktop() -> bool {
    ...
    combined.contains("unity") || combined.contains("ubuntu")
}
    ...
    fn position_and_show(window: &WebviewWindow, app: &AppHandle) {
        ...
        std::thread::spawn(move || {
            ...
            if is_wayland_session {
                ...
                if is_unity_or_ubuntu_desktop() {
                    // Unity/Ubuntu sessions can throw X11 BadImplementation on aggressive
                    // focus activation flows. Keep it simple and stable.
    ...
    tauri::Builder::default()
        ...
        .plugin(tauri_plugin_single_instance::init(|app, argv, _cwd| {
            ...
            let _ = app.run_on_main_thread(move || {
                let event_kind = detect_shortcut_event_kind(&app_handle, argv_cloned.as_slice());

                if !should_handle_shortcut_event(event_kind) {
                    println!(
                        "[SingleInstance] Duplicate shortcut launch ignored (bounce={}ms, same-action={}ms, kind={})",
                        SHORTCUT_BOUNCE_GUARD_MS,
                        SHORTCUT_SAME_ACTION_DEBOUNCE_MS,
                        event_kind
                    );
                    return;
                }
                ...
                if argv_cloned.iter().any(|arg| arg == "--settings") {
                    println!(
                        "[SingleInstance] Secondary instance with --settings flag, opening settings..."
                    );
                    SettingsController::show(&app_handle);
                } else if argv_cloned.iter().any(|arg| arg == "--emoji") {
                    println!(
                        "[SingleInstance] Secondary instance with --emoji flag, opening emoji picker..."
                    );
                    WindowController::toggle_with_tab(&app_handle, Some("emoji"));
                } else {
                    println!("[SingleInstance] Secondary instance detected, showing/focusing window...");
